...
import h5py
from keras.layers import Concatenate, Multiply ## 新导入的包
...
class VASC:
    ...
    def vaeBuild( self ):
        ...
        expr_x_drop_p = Lambda( lambda x:K.exp(x) )(expr_x_drop)  ## p
        expr_x_nondrop_p = Lambda( lambda x:1-x )( expr_x_drop_p )  ## q
        expr_x_nondrop_log = Lambda( lambda x:K.log(x+1e-20) )(expr_x_nondrop_p)  ## log q
        expr_x_drop_log = Lambda( lambda x:K.log(x+1e-20) )(expr_x_drop_p)  ## log p      
        expr_x_drop_log = Reshape( target_shape=(self.in_dim,1) )(expr_x_drop_log)  ## log p shape = (in_dim, 1)
        ...
        logits = Concatenate(axis=-1)( [expr_x_drop_log,expr_x_nondrop_log] ) # 原来是上面那行，改成这行， logits shape = (in_dim, 2)  
        temp_in = Input( shape=(self.in_dim,) ) # shape=(in_dim, 1)
        temp_ = RepeatVector( 2 )(temp_in) ## 将 temp_in 重复两次 shape = (in_dim, 2, 1 )
        print(temp_.shape)
        temp_ = Permute( (2,1) )(temp_)  ## shape = (in_dim, 1 , 2)
...
##        #print(samples.shape)
        ...
        out = Multiply()([expr_x, samples]) # 原来是上面那行，改成这行。逐元素相乘积
...
def vasc( expr,
    ...
    ============
    Parameters:
        expr: expression matrix (n_cells * n_features)                                              ## 表达矩阵，细胞×基因
        epoch: maximum number of epochs, default 5000                                               ## 最大epochs，默认5000
        latent: dimension of latent variables, default 2                                            ## 隐变量的维度，默认2
        patience: stop if loss showes insignificant decrease within *patience* epochs, default 50   ## 如果loss在50个周期内没有明显减少，则停止训练
        min_stop: minimum number of epochs, default 500                                             ## 最小epochs，默认500
        batch_size: batch size for stochastic optimization, default 32                              ## 随机优化的 batch size 默认 32
        var: whether to estimate the variance parameters, default False                             ## 是否估计 variance参数，默认False
        prefix: prefix to store the results, default 'test'                                         ## 储存结果的前缀，默认'test'
        label: numpy array of true labels, default None#                                            ## 数据标签，numpy array，默认None
        log: if log-transformation should be performed, default True                                ## 是否进行对数转换，默认True
        scale: if scaling (making values within [0,1]) should be performed, default True            ## 是否将值限制在[0,1]之间，默认True
        annealing: if annealing should be performed for Gumbel approximation, default False         ## 对于 Gumbel近似，是否进行退火，默认False
        tau0: initial temperature for annealing or temperature without annealing, default 1.0       ## 退火的初始温度或不退火的温度，默认1.0
        min_tau: minimal tau during annealing, default 0.5                                          ## 退火时的最低温度，默认0.5
        rep: not used                                                                               ## 没用到
    ...
    print('数据：{} × {}'.format(expr.shape[0],expr.shape[1]) )
    print('参数 | epoch: {} | patience: {} | batch_size: {} | latent: {} '.format(epoch,patience,batch_size,latent))
    expr[expr<0] = 0.0
...
#            label = label[o==1]
    ...
    for e in range(1, epoch+1):
        ...
        #                     )
        loss_ = vae_.vae.fit( [expr_train,tau_in],epochs=1,batch_size=batch_size,
                             shuffle=True,verbose=0
                             ) # 原来是上面那行，改成这行。
    ...
    pd.DataFrame(loss).to_csv('loss.csv')
