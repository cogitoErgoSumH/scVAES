...
# express data
X = pd.read_csv("../mouse_cell_atlas_filteredcell2_2000.csv", index_col=0)
expression_data = np.array(X, dtype=np.int)
#selected = np.std(expression_data, axis=0).argsort()[-2000:][::-1]
#expression_data = expression_data[:, selected]

clusters = pd.read_csv("../mouse_cell_atlas_filteredcell2_labels.csv", index_col=0)
