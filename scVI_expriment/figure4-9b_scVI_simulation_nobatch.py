...
# express data
X = pd.read_csv("../datasets/simulation.csv", index_col=0)
...
clusters = pd.read_csv("../datasets/simulation_labels.csv", index_col=0)
cell_type = np.array(clusters['cell_type'])
ct = clusters['cell_type']

# In[8]:


from sklearn.model_selection import train_test_split
expression_train, expression_test, c_train, c_test = train_test_split(X, ct, random_state=0, test_size=0.01)
...
model = scVI.scVIModel(expression= expression, n_latent=10, n_layers=1,
                      # batch_ind=batch_ind, num_batches=2,
                       kl_scale=kl_scalar,
...
plt.figure(figsize=(8, 8))
plt.scatter(tsne[:,0],tsne[:,1], c=b_train, cmap=plt.get_cmap('brg', 2), edgecolors='none', s=20)
plt.axis("off")
plt.tight_layout()
