...
def show_tSNE(latent, labels, return_tSNE=False):
    ...
    for i, v in enumerate(unique_label):
        plt.scatter(latent_df['tsne1'][latent_df['labels']==v], latent_df['tsne2'][latent_df['labels']==v],
                    label=v, c=colors[i], edgecolors='none')
...
def _save_result(x, y, model, out_dir, name):
    ...
    #x_hat.to_csv("x_hat_simulation" + args.recon_distribution + ".csv")
    ...
    ##批次效应散点图
    batch = pd.read_csv('../simulation_labels.csv',index_col=0)['batch'].values
    print("batch cluster silhouette_score =", silhouette_score(tsne, batch))
    plt.figure(figsize=(8, 8))
    plt.scatter(tsne[:,0],tsne[:,1], c=batch, cmap=plt.get_cmap('brg', 2), edgecolors='none')
    plt.axis("off")
    plt.tight_layout()
...
class analysis_parser():
    def __init__(self):
        self.data_matrix_file = '../datasets/simulation.csv'
        self.config_file = 'model_config.yaml'  ## 这里面改隐空间的维度参数
        self.pretrained_model_file = None
        self.normalize = None
        self.data_label_file = '../datasets/simulation_labels.csv'
        self.verbose = 1
        self.verbose_interval = 10
        self.show_plot = 0
        self.out_dir = './'
        self.recon_distribution = 'student' ## 这里修改为student,zinb
