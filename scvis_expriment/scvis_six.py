...
def _save_result(x, y, model, out_dir, name):
    ...
    x_hat.to_csv("x_hat_six_050_" + args.recon_distribution + ".csv")
...
class analysis_parser():
    def __init__(self):
        self.data_matrix_file = '../datasets/sixgroupsimulation_0.50dropout.csv'
        self.config_file = 'model_config.yaml'  ## 这里面改隐空间的维度参数
        self.pretrained_model_file = None
        self.normalize = None
        self.data_label_file = '../datasets/sixgroupsimulation_labels.csv'
        self.verbose = 1
        self.verbose_interval = 10
        self.show_plot = 0
        self.out_dir = './'
        self.recon_distribution = 'zinb' ## 修改为 gaussian、student、nb或zinb
