...
class SCVIS(object):
    def __init__(self, architecture, hyperparameter):
        ...
        self.latent[' '] = tf.sqrt(self.latent['sigma_square'])
...
def train(args):
    ...
    root_path_one='C:/Users/jiangchengke/PycharmProjects/test_gpu/testcpa'
    root_path = root_path_one+'/gene_ex_df_subs_raw_'
    # 以下是生成dendritic的数据
    save_name='dendritic'
    dimension_li = range(9000, 9200, 100)

    # 以下是生成bone_marrow的数据
    save_name='bone_marrow'
    dimension_li = range(5000, 5200, 100)

    # 以下是mesc的数据
    save_name='mesc'
    dimension_li = range(6100, 6500, 100)




    # dimension_li=[720]
    for i in dimension_li:
        save_file_name = root_path_one + '/vae_data_{}.csv'.format(save_name+str(i))
        save_name_two=save_name+str(i)
        file_name=root_path+'{}.csv'.format(save_name_two)
        x, y, architecture, hyperparameter, train_data, model, normalizer, out_dir, name = _init_model(
        ...
        _save_result(x, y, model, out_dir, name,file_name,save_file_name)
        # plt.show()
...
def _init_model(args, mode,file_name=None):
    ##x = pd.read_csv(args.data_matrix_file, sep='\t').values # 读取数据
    # x = pd.read_csv(args.data_matrix_file, sep=',', index_col=0).values # 读取数据
    # x=pd.read_csv('C:/Users/jiangchengke/PycharmProjects/test_gpu/testcpa/gene_ex_df_subs_raw_{}.csv'.format(index_i)).T.values
    x = pd.read_csv(file_name).T.values

    # 以下是获得成对的基因，然后用成对的基因进行降维
    # x_two=pd.read_csv('C:/Users/jiangchengke/PycharmProjects/test_gpu/testcpa/gene_ex_df_subs_raw_{}.csv'.format(index_i))
    # x_two_generate=get_gene_name_label(x_two,index_i)
    # print(x_two_generate)
    # labels_li=x_two_generate[x_two_generate.shape[1]-1].values
    #
    # x_two_generate.drop(x_two_generate.shape[1]-1,axis = 1,inplace=True)
    # x=x_two_generate.values
# =============================================================================
...
def show_tSNE(latent, labels, return_tSNE=False):
    ...
    for i, v in enumerate(unique_label):
        plt.scatter(latent_df['tsne1'][latent_df['labels']==v], latent_df['tsne2'][latent_df['labels']==v],
                    label=v, c=colors[i], edgecolors='none', s=10)
...
# 将输出结果保存为文件
def _save_result(x, y, model, out_dir, name,file_name,save_name):
    x_hat = model.encode_decode(x)[2]
    x_hat = pd.DataFrame(x_hat)
    
    x_hat.to_csv("x_hat_two_050_" + args.recon_distribution + ".csv")
    
    z_mu, _ = model.encode(x)
    #以下是将数据重新存为基因和表达的excel文件
    # x_ = pd.read_csv('C:/Users/jiangchengke/PycharmProjects/test_gpu/testcpa/gene_ex_df_subs_raw_20.csv')
    #以下是训练mesc的文件
    x_ = pd.read_csv(file_name)


    x_columns=x_.columns
    new_x_=pd.DataFrame(data=z_mu.T,columns=x_columns)
    print(new_x_.shape)
    # new_x_.to_csv('C:/Users/jiangchengke/PycharmProjects/test_gpu/testcpa/vae_data_two_{}.csv'.format(index_i))
    new_x_.to_csv(save_name)
    ...
    # print(type(z_mu))
    # print(type(y))
    # train_data=pd.DataFrame(z_mu,columns=range(0,z_mu.shape[1]))
    # train_data[z_mu.shape[1]]=y
    # print(train_data.shape)
    # print(train_data.head())
    # # train_model(1024,0,train_data)
    # train_data.to_csv('./data/two_gene_.csv'.format(index_i),index=False)
    # tsne = show_tSNE(z_mu, ct, return_tSNE=True)
    

# =============================================================================
#      ##批次效应散点图
#     batch = pd.read_csv('../sixgroupsimulation_labels.csv',index_col=0)['batch'].values
...
class analysis_parser():
    def __init__(self):
        self.data_matrix_file = '../datasets/twogroupsimulation_0.50dropout.csv'
        self.config_file = 'model_config.yaml'  ## 这里面改隐空间的维度参数
        self.pretrained_model_file = None
        self.normalize = None
        self.data_label_file = '../datasets/twogroupsimulation_labels.csv'
        self.verbose = 1
        self.verbose_interval = 10
        self.show_plot = 0
        self.out_dir = './'
        self.recon_distribution = 'student' ## 修改为 gaussian、student、nb或zinb

def get_gene_name_label(gene_expression,dimention_count):
    ...
    s.close()
    i=0
    for gene_pair in gene_pair_label:
        if i>10:
            break
        # i+=1
        x_gene_name,y_gene_name,label=gene_pair.strip().split()

        # x_gene_ensembl = np.log10(gene_expression[x_gene_name].values +1)/max(gene_expression[x_gene_name].values)
        # y_gene_ensembl = np.log10(gene_expression[y_gene_name].values + 1)/max(gene_expression[x_gene_name].values)
        # x_gene_ensembl, y_gene_ensembl=gene_expression[x_gene_name].values,gene_expression[y_gene_name].values
        x_gene_ensembl = np.log10(gene_expression[x_gene_name].values +1)
        y_gene_ensembl = np.log10(gene_expression[y_gene_name].values + 1)
        # print(type(x_gene_ensembl))
        # print(type(y_gene_ensembl))
        gene_name_exp.append([x_gene_name,y_gene_name,x_gene_ensembl,y_gene_ensembl,label])

        x_gene_ensembl=list(x_gene_ensembl)
        y_gene_ensembl=list(y_gene_ensembl)
        # x_gene_ensembl=[round(el_x,2) for el_x in x_gene_ensembl]
        # x_gene_ensembl.append(100)
        # y_gene_ensembl=[round(el_y,2) for el_y in y_gene_ensembl]
        x_gene_ensembl.extend(y_gene_ensembl)
        x_gene_ensembl.append(int(label))
        gene_name_exp_train.append(x_gene_ensembl)
    with open('./gene_name_exp.pkl', 'wb') as f:
        pickle.dump(gene_name_exp, f)
    # print(len(gene_name_exp))
    gene_name_exp_df=pd.DataFrame(gene_name_exp)
    # print(gene_name_exp_df.iloc[0,2])
    # gene_name_exp_df.to_csv('./gene_name_exp_df.csv',index=False)
    print(gene_name_exp_df.head())
    gene_name_exp_train_df=pd.DataFrame(gene_name_exp_train)
    # gene_name_exp_train_df.to_csv('./gene_name_exp_train_df_{}.csv'.format(dimention_count),index=False)
    print(gene_name_exp_train_df.head())
    return gene_name_exp_train_df
# In[11]:
...
def train_model(batch_size,dimention_count=None,all_data=None):
    ...
    train_data_last_index =all_data.shape[1] - 1
